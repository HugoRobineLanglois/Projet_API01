...
ALTER TABLE `booksmatching`.`note` 
ADD COLUMN `date` VARCHAR(20) NULL AFTER `validate`,
ADD COLUMN `comment` VARCHAR(255) NULL AFTER `date`;

insert into booksmatching.note (idBook, idUser) values (2,1),(3,1);
insert into booksmatching.note(idBook, idUser) value (4,1);